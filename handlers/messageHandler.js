...
function setupMessageHandler(bot, { callGemini }) {
  bot.on('message', async (msg) => {
    ...
    // Игнорируем сообщения, начинающиеся с иконок команд
    const MENU_CMDS = ['📊', '📝', '🤲', '💀', '💰', '🧠', '🏆', '❓'];
    ...
    try {
      ...
      // Экранируем весь ответ, но оставляем эмодзи и заголовок без экранирования
      const fullText = `🤖 ${escapeMarkdown(reply)}`;
      const parts = splitMessage(fullText);

      // Редактируем первое сообщение (загрузку) первой частью без Markdown
      try {
        await bot.editMessageText(parts[0], {
          chat_id: msg.chat.id,
          message_id: loading.message_id
          // parse_mode не передаём, чтобы избежать проблем с Markdown
        });
      } catch (editErr) {
        // Если редактирование не удалось (слишком длинное или ошибка), удалим загрузку и отправим новое
