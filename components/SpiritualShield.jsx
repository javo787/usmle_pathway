...
function ZikrModal({ zikrs, onClose, onUpdate, theme }) {
  ...
  return (
    <div className="fixed inset-0 flex items-end justify-center" style={{ zIndex: 9999 }}>
      ...
      />
      <div
        className={`relative w-full max-w-md rounded-t-3xl p-6 pb-10 ${bgColor} border-t ${borderColor}`}
        style={{ zIndex: 1 }}
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <button
            onPointerDown={(e) => { e.stopPropagation(); onClose(); }}
            className={`w-10 h-10 rounded-2xl flex items-center justify-center opacity-60 hover:opacity-100 transition border ${theme.input}`}
          >
            <X size={18}/>
          </button>
        </div>
              ...
              <div key={z.id} className={`rounded-2xl p-3 border ${theme.input}`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1.5 ml-2">
                    <button onPointerDown={() => updateCount(z.id, -1)} className={`w-8 h-8 rounded-xl flex items-center justify-center border transition ${theme.input}`}>
                      <Minus size={13}/>
                    </button>
                    <span className={`text-sm font-black w-9 text-center ${theme.text}`}>{z.count}</span>
                    <button onPointerDown={() => updateCount(z.id, 1)} className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold transition ${theme.button}`}>
                      <Plus size={13}/>
                    </button>
                    <button onPointerDown={() => removeZikr(z.id)} className="w-6 h-6 flex items-center justify-center opacity-30 hover:opacity-70 ml-1">
                      <X size={11}/>
                    </button>
                  </div>
                </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`w-11 h-11 rounded-2xl flex items-center justify-center font-bold transition ${newLabel.trim() ? theme.button : theme.input + ' opacity-40 border'}`}
          >
            <Plus size={18}/>
          </button>
        </div>
