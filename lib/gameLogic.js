export const calculateScore = (data, goals) => {
  ...
  const identityBonus = (data.planning.identityStatement && data.planning.identityStatement.trim().length > 0) ? 3 : 0;
  const muhasabaBonus = (data.spiritual.muhasaba && data.spiritual.muhasaba.trim().length > 0) ? 3 : 0;
  const coveyBonus = (data.planning.coveyMatrix?.Q2?.length > 0) ? 5 : 0;
  const worryBonus = (data.planning.worryAnalysis?.worry?.trim().length > 0) ? 2 : 0;

  const planningScore = (hasSchedule ? 10 : 0) + coreIdeaBonus + identityBonus + coveyBonus + worryBonus;
  const totalSpiritualScore = spiritualScore + muhasabaBonus;

  return Math.min(100, Math.round(academicScore + totalSpiritualScore + sportScore + planningScore));
};
